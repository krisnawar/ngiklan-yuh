...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
        ...
        $this->forgot_identity_column = $this->config->item('forgot_identity', 'ion_auth');
	...
	public function reset_password($identity, $new) {
		...
		if (!$this->forgot_identity_check($identity)) {
			$this->trigger_events(['post_change_password', 'post_change_password_unsuccessful']);
			return FALSE;
		}

		$return = $this->forgot_set_password_db($identity, $new);
    ...
    public function forgot_identity_check($identity = '')
	{
		$this->trigger_events('identity_check');
		...
		return $this->db->where($this->forgot_identity_column, $identity)
						->limit(1)
						->count_all_results($this->tables['users']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['users'], $update, [$this->forgot_identity_column => $identity]);
    ...
    public function register_admin($identity, $password, $email, $additional_data = [], $groups = [])
	{
		$this->trigger_events('pre_register');
		...
		else if (!$this->config->item('admin_group', 'ion_auth') && empty($groups))
		{
			$this->set_error('account_creation_missing_default_group');
			return FALSE;
		}
		...
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'], ['name' => $this->config->item('admin_group', 'ion_auth')], 1)->row();
		if (!isset($query->id) && empty($groups))
		{
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
		...
		// add in groups array if it doesn't exists and stop adding into default group if default group ids are set
		if (isset($admin_group->id) && empty($groups))
		{
    ...
    protected function forgot_set_password_db($identity, $password)
	{
		$hash = $this->hash_password($password, $identity);
		...
		$this->db->update($this->tables['users'], $data, [$this->forgot_identity_column => $identity]);
