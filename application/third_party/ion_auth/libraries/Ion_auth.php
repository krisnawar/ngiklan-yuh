...
class Ion_auth
{
	...
	public function forgotten_password($identity)
	{
		...
		$user = $this->where($this->ion_auth_model->forgot_identity_column, $identity)
					 ->where('active', 1)
					 ->users()->row();
	...
	public function forgotten_password_check($code)
	{
		...
		else
		{
            if ($this->config->item('forgot_password_expiration', 'ion_auth') > 0)
			{
				...
				{
					//it has expired
					$identity = $user->{$this->config->item('forgot_identity', 'ion_auth')};
    ...
    public function register_admin($identity, $password, $email, $additional_data = [], $group_ids = [])
	{
		$this->ion_auth_model->trigger_events('pre_account_creation');
    ...
    public function is_superadmin($id = FALSE)
	{
		...
		return $this->ion_auth_model->in_group($superadmin_group, $id);
	}
